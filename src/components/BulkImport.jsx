...
export default function BulkImport({
  ...
  const handleParse = () => {
    ...
    if (headers.length === 0 || rows.length === 0) {
      setError(
        'Could not parse data. Make sure the first row is a header and there is at least one data row.'
      );
      setParsed(null);
      return;
    }
    ...
    if (mode === 'menu') {
      ...
      if (missing.length) {
        setError(
          `Missing required column(s): ${missing.join(
            ', '
          )}. Required: group_name, item_name, price`
        );
        setParsed(null);
        return;
      }
    ...
    try {
      if (mode === 'menu') {
        const records = parsed.rows
          .filter((r) => r.name)
          .map((r) => {
            const status = STOCK_OPTIONS.includes(r.stock_status)
              ? r.stock_status
              : 'available';

            const cat = categories.find((c) => c.name === r.category);

            const tagNames = (r.menu_tags || '')
              .split(/[|,]/)
              .map((s) => s.trim())
              .filter(Boolean);
            ...
            const menu_tag_ids = tagNames
              .map(
                (n) =>
                  tags.find(
                    (tg) =>
                      tg.name.toLowerCase() === n.toLowerCase()
                  )?.id
              )
              .filter(Boolean);

            const groupNames = (r.addon_groups || '')
              .split(/[|,]/)
              .map((s) => s.trim())
              .filter(Boolean);
            ...
            const addon_group_ids = groupNames
              .map(
                (n) =>
                  groups.find(
                    (g) =>
                      g.name.toLowerCase() === n.toLowerCase()
                  )?.id
              )
              .filter(Boolean);
        ...
        // Auto-create Category records for any category names
        ...
        if (newCatNames.length > 0) {
          ...
          const newCategories = newCatNames.map((n, i) => ({
            name: n,
            name_th: '',
            order: baseOrder + i,
          }));
        ...
        setResult(
          `${records.length} menu item${
            records.length > 1 ? 's' : ''
          } imported${
            newCatNames.length
              ? ` · ${newCatNames.length} new categor${
                  newCatNames.length > 1 ? 'ies' : 'y'
                }`
              : ''
          }.`
        );
        ...
        const records = Object.values(groupMap).filter(
          (g) => g.name
        );
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-stone-400 px-1">
        Paste CSV data with a header row to create many items or
        add-on groups at once. Use the sample to see the expected
        format.
      </p>
        ...
        <div className="text-[11px] text-stone-400 leading-relaxed">
          {mode === 'menu' ? (
            <p>
              ...
              . Optional:{' '}
              <code className="text-stone-600">
                name_th, description, description_th, image_url
              </code>{' '}
              (prefer square 1:1, ~800px, max 1500px)
              <code className="text-stone-600">
                , category, stock_status
              </code>{' '}
              (available | limited | sold_out_today |
              not_available),{' '}
              <code className="text-stone-600">
                menu_tags
              </code>{' '}
              (pipe- or comma-separated tag names, e.g.{' '}
              <code className="text-stone-600">
                Spicy|Vegetarian
              </code>
              ),{' '}
              ...
              (pipe- or comma-separated group names, e.g.{' '}
              <code className="text-stone-600">
                Toppings|Extras
              </code>
              ). Category Thai name is pulled from the existing
              category.
            </p>
          ) : (
            <p>
              Required columns:{' '}
              <code className="text-stone-600">
                group_name, item_name, price
              </code>
              . Optional:{' '}
              <code className="text-stone-600">
                group_name_th, item_name_th
              </code>
              . Multiple rows with the same{' '}
              <code className="text-stone-600">
                group_name
              </code>{' '}
              become items in that group.
            </p>
          )}
        </div>
              ...
              <thead className="bg-stone-50 sticky top-0">
                <tr>
                  {parsed.headers.map((h) => (
                    <th
                      key={h}
                      className="text-left font-semibold text-stone-500 px-3 py-2 whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
