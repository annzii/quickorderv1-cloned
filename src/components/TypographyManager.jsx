...
export default function TypographyManager({ settings, onSaved }) {
  ...
  useEffect(() => {
    ...
    setDraft({
      en: { ...EN_DEFAULTS, ...(typo.en || {}) },
      th: { ...TH_DEFAULTS, ...(typo.th || {}) },
    });
  }, [settings]);

  useEffect(() => {
    if (draft) applyPreview(draft);
  }, [draft, applyPreview]);
  ...
  const save = async () => {
    ...
    try {
      ...
      alert(error.message || 'Failed to save typography.');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!settings) {
    return (
      <p className="text-center text-stone-400 text-sm py-16">
        Save store settings first to manage typography.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-stone-400 px-1">
        Changes preview live across the app. Save to persist.
      </p>
      ...
      {['en', 'th'].map((lang) => (
        ...
        >
          <h3 className="text-sm font-bold text-stone-900 mb-3">
            {lang === 'en' ? 'English' : 'Thai'} Typography
          </h3>

          <TypographyControls
            lang={lang}
            values={draft[lang]}
            onChange={(field, val) => update(lang, field, val)}
          />
        </div>
      ))}
