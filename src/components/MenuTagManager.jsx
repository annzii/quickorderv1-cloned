...
import { Plus, Pencil, Trash2, X, Save, ArrowUp, ArrowDown, Check } from 'lucide-react';
...
export default function MenuTagManager({ onSaved }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error('Failed to load menu tags:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (t) => {
    setDraft({
      ...
      icon_url: t.icon_url || '',
      order: t.order ?? 0,
      is_filterable: t.is_filterable || false,
    });
    setEditing(t.id);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      console.error('Failed to save menu tag:', error);
      alert(error.message || 'Failed to save menu tag.');
    }
  };
  ...
  const remove = async (id) => {
    ...
    try {
      ...
      console.error('Failed to delete menu tag:', error);
      alert(error.message || 'Failed to delete menu tag.');
    }
  };
  ...
  const move = async (idx, dir) => {
    ...
    try {
      ...
      console.error('Failed to reorder menu tags:', error);
      alert(error.message || 'Failed to reorder menu tags.');
      await load();
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl max-h-[92vh] overflow-y-auto">
            ...
            <div className="p-4 space-y-3">
              <input
                placeholder="Tag name (EN) — e.g. Spicy"
                value={draft.name}
                onChange={(e) =>
                  setDraft({
                    ...draft,
              ...
              <input
                placeholder="Tag name (TH) — e.g. เผ็ด"
                value={draft.name_th}
                onChange={(e) =>
                  setDraft({
                    ...draft,
              ...
              <input
                placeholder="Icon image URL (png/svg)"
              ...
              >
                ...
                <span className="text-sm text-stone-700 text-left">
                  Show as collection filter pill in storefront
                </span>
              </button>
